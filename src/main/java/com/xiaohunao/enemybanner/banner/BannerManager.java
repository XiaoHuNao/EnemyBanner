...
public class BannerManager {
    ...
    private static final List<Map.Entry<Level, BlockPos>> bannerBlockList;

    static {
        bannerBlockList = new ArrayList<>();
    }
    ...
    public static void onServerTick(ServerTickEvent.Pre event){
        bannerBlockList.removeIf(blockEntry -> blockEntry.getKey() == null || blockEntry.getKey().getBlockState(blockEntry.getValue()).isAir());
        bannerEffect();
        for (Map.Entry<Level, BlockPos> bannerEntry : bannerBlockList) {
            if (bannerEntry.getKey().getBlockEntity(bannerEntry.getValue()) instanceof EnemyBannerBlockEntity blockEntity){
                String silksId = blockEntity.getParameters().getSilksId();
                BannerBehavior bannerBehavior = BannerBehaviorProvider.getInstance().getBannerBehavior(silksId);
                bannerBehavior.onTick(bannerEntry.getKey(), bannerEntry.getValue(), event.getServer().getTickCount());
            }
        }
    }

    private static void bannerEffect(){
        for (Map.Entry<Level, BlockPos> blockEntry : bannerBlockList) {
            if (blockEntry.getKey() != null){
                List<Player> playersInRange = new ArrayList<>();
                for (Player player : blockEntry.getKey().players()) {
                    BlockPos blockPos = player.blockPosition();
                    if (
                            Math.abs(blockPos.getX() - blockEntry.getValue().getX()) < BASIC_RANGE  &&
                                    Math.abs(blockPos.getY() - blockEntry.getValue().getY()) < BASIC_RANGE &&
                                    Math.abs(blockPos.getZ() - blockEntry.getValue().getZ()) < BASIC_RANGE
                    ){
                        playersInRange.add(player);
                    }
                }
                for (Player player : playersInRange) {
                    if (!player.hasEffect(EnemyBanner.ENEMY_BANNER_EFFECT)) {
                        MobEffectInstance mobEffectInstance = new MobEffectInstance(EnemyBanner.ENEMY_BANNER_EFFECT, MobEffectInstance.INFINITE_DURATION, 0, false, false, true);
    ...
    public static List<BlockPos> getBannerOfEntity(Entity entity){
        ...
        for (Map.Entry<Level, BlockPos> bannerEntry : bannerBlockList) {
            if (bannerEntry.getKey().dimension().equals(entity.level().dimension()) && isInBannerRange(bannerEntry.getValue(), entity))
    ...
    public static boolean isInBannerRange(BlockPos bannerPos, Entity entity){
        ...
        if (
            Math.abs(bannerPos.getX() - entity.blockPosition().getX()) < range &&
    ...
    public static BlockPos getRecentBanner(Level level, BlockPos pos){
        ...
        for (Map.Entry<Level, BlockPos> blockEntity : bannerBlockList) {
            double v = blockEntity.getValue().getCenter().distanceTo(pos.getCenter());
            if (level.dimension().equals(blockEntity.getKey().dimension()) && v < min){
                min = v;
    ...
    public static void add(Map.Entry<Level, BlockPos> blockEntity){
        if (!bannerBlockList.contains(blockEntity))
            bannerBlockList.add(blockEntity);
    }
